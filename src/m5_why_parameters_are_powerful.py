...
def main():
    ...
    better_draw_circles(rg.Point(-200, 0), 8)
    even_better_draw_circles((rg.Point(100, 50)), 20, 10, 'green', 4)
...
def better_draw_circles(point, radiis):
    """
    ...
    for k in range(1, 11):  # k becomes 1, 2, 3, ... 10
        ...
        # Go DOWN 15 pixels, ending up pointing east again
        turtle.right(90)
        turtle.forward(radiis)
        turtle.left(90)

        turtle.pen_down()
        turtle.draw_circle(radiis * k)# Radius 15, 30, 45, 60, ...
...
def even_better_draw_circles(point, radiis, cycle, colors, thickness):
    ...
    turtle = rg.SimpleTurtle()
    turtle.pen_up()
    turtle.pen = rg.Pen(colors, thickness)
    turtle.go_to(point)
    turtle.set_heading(0)  # Point "east" (towards the right)

    for k in range(1, cycle + 1):  # k becomes 1, 2, 3, ... 10
